...
databaseTest("Storage upload target: returns storagePath and optional publicUrl", async () => {
  ...
  const res = await fetch(`${Deno.env.get("TEST_BASE_URL") || "http://localhost:8000"}/api/jobs/${job.id}/photo/upload-target`, {
    method: "POST",
