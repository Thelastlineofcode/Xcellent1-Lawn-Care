...
Deno.test("RLS migration contains ENABLE and CREATE POLICY statements", async () => {
  const txt = await Deno.readTextFile("./db/migrations/2025-12-10-0002-rls-policies.sql");
  assert(txt.includes("ENABLE ROW LEVEL SECURITY"), "RLS enable statements missing");
