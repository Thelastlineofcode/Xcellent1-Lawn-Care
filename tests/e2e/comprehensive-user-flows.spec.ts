...
class TestFixture {
  ...
  trackUser(id: string) { this.createdUsers.push(id); }
  trackClient(id: string) { this.createdClients.push(id); }
  trackJob(id: string) { this.createdJobs.push(id); }
  trackInvoice(id: string) { this.createdInvoices.push(id); }
}
...
Deno.test("Owner Workflow: Complete Business Management Journey", async (t) => {
  ...
  await t.step("Owner can record payments on invoices", async () => {
    ...
    const response = await fetch(`${BASE_URL}/api/owner/invoices/test-invoice-id/payment`, {
      method: "POST",
      headers: { 
        "Authorization": `Bearer ${token}`,
        "Content-Type": "application/json"
...
Deno.test("Client Workflow: Self-Service Portal Journey", async (t) => {
  ...
  await t.step("Client can view upcoming scheduled jobs", async () => {
    const response = await fetch(`${BASE_URL}/api/client/jobs?status=scheduled`, {
      headers: { "Authorization": `Bearer ${token}` }
    });
    // Expected to fail initially - implementation needed
  ...
  await t.step("Client can view service history and photos", async () => {
    const response = await fetch(`${BASE_URL}/api/client/jobs?status=completed`, {
      headers: { "Authorization": `Bearer ${token}` }
    });
    // Expected to fail initially - implementation needed
    assertEquals(response.status, 200, "Should return completed jobs with photos");
  });

  await t.step("Client can view outstanding invoices", async () => {
    const response = await fetch(`${BASE_URL}/api/client/invoices?status=outstanding`, {
      headers: { "Authorization": `Bearer ${token}` }
    });
    // Expected to fail initially - implementation needed
  ...
  await t.step("Client can make online payments", async () => {
    ...
    const response = await fetch(`${BASE_URL}/api/client/invoices/test-invoice-id/payment`, {
      method: "POST",
      headers: { 
        "Authorization": `Bearer ${token}`,
        "Content-Type": "application/json"
  ...
  await t.step("Client can mark payments as completed", async () => {
    const response = await fetch(`${BASE_URL}/api/client/invoices/test-invoice-id/mark-paid`, {
      method: "PATCH",
      headers: { 
        "Authorization": `Bearer ${token}`,
        "Content-Type": "application/json"
...
Deno.test("Guest Workflow: Public Access and Lead Generation", async (t) => {
  ...
  await t.step("Visitors can access careers page", async () => {
    ...
    assertEquals(html.includes("Xcellent1"), true, "Should contain company branding");
  });
  ...
  await t.step("Visitors can view service information", async () => {
    ...
    assertEquals(mainResponse.status, 200, "Should serve main page with service info");
  });
});
...
Deno.test("Performance: Response Times Within Limits", async (t) => {
  ...
  await t.step("Quote calculator responds within 2 seconds", async () => {
    ...
    assertEquals(responseTime < 2000, true, `Response time ${responseTime}ms should be under 2000ms`);
  });
  ...
  await t.step("Dashboard loads within acceptable time", async () => {
    ...
    assertEquals(responseTime < 1000, true, `Dashboard load time ${responseTime}ms should be under 1000ms`);
  });
});
