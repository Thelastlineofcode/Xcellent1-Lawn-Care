...
databaseTest("RLS integration - owners and crew access", async () => {
  ...
  const jobA = await testFixture.createTestJob(clientA.id, { crew_id: crew.id });
  ...
  const resB = await db.queryObject(`SELECT id FROM jobs WHERE client_id = $1`, [clientA.id]);
  ...
  const resA = await db.queryObject(`SELECT id FROM jobs WHERE client_id = $1`, [clientA.id]);
  await db.queryObject("COMMIT");
  ...
  const resCrew = await db.queryObject(`SELECT id FROM jobs WHERE crew_id = $1`, [crew.id]);
  await db.queryObject("COMMIT");
