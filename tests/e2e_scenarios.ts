...
if (!JWT_SECRET) {
  console.error(
    "❌ E2E Test skipped: SUPABASE_JWT_SECRET not found in .env.local",
  );
  Deno.exit(1);
}
...
if (DATABASE_URL) {
  try {
    ...
    console.log(
      `   ✅ Seeded Test Users in DB (Owner: ${ownerId}, Crew: ${crewId})`,
    );
...
Deno.test("Scenario 1: Owner creates Client, Job, and Invoice", async (t) => {
  ...
  await t.step("Owner creates a Client", async () => {
    ...
    if (res.status === 401) {
      console.log(
        "⚠️  401 Unauthorized - The test user ID doesn't exist in DB. Skipping.",
      );
      return;
    }
...
Deno.test("Scenario 3: Client pays Invoice", async (t) => {
  ...
  await t.step("Owner records payment manually", async () => {
    const res = await fetch(
      `${BASE_URL}/api/owner/invoices/${createdInvoiceId}/payment`,
      {
        method: "POST",
        headers: {
          "Authorization": `Bearer ${OWNER_TOKEN}`,
