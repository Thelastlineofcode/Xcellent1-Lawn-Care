...
if (document.getElementById('dashboard')) {
  ...
  function updateStats(data) {
    ...
    const photos = events.filter(e => e.payload && e.payload.photo_path).length;
  ...
  function renderLeads(leads) {
    if (leads.length === 0) {
      leadsContainer.innerHTML = '<div class="empty-state"><p class="text-muted">💼 No applications yet. <a href="/">Share careers page</a>.</p></div>';
      return;
    }
    ...
    if (careerApps.length > 0) {
      html += '<h3 class="mb-sm">💼 Worker Applications (' + careerApps.length + ')</h3>';
      html += careerApps.map(lead => `
    ...
    if (otherLeads.length > 0) {
      html += '<h3 class="mt-lg mb-sm">🎯 Other Inquiries (' + otherLeads.length + ')</h3>';
      html += otherLeads.map(lead => `
        <div class="card"><div class="lead-card"><div class="lead-info">
          <h3>👤 ${escapeHTML(lead.name)}</h3>
          <div class="lead-meta">
            <span>📞 ${escapeHTML(lead.phone)}</span><span>•</span>
  ...
  function renderOutbox(events) {
    if (events.length === 0) {
      outboxContainer.innerHTML = '<div class="empty-state"><p class="text-muted">📋 No pending work queue items.</p></div>';
      return;
    }
    ...
    Object.entries(grouped).forEach(([type, typeEvents]) => {
      ...
      typeEvents.forEach(event => {
        ...
        const statusClass = event.status === 'processed' ? 'status-complete' : 'status-pending';
  ...
  window.refreshDashboard = async function() {
    try {
      ...
      showMessage('dashboard-status', `⚠ Unable to load data: ${err.message}. Retrying in 20 seconds...`, 'error', 0);
    }
  };
...
if (document.getElementById('upload-form')) {
  ...
  fileInput.addEventListener('change', (e) => {
    ...
    if (file.size > MAX_FILE_SIZE) {
      showMessage('upload-status', `File too large (${(file.size / 1024 / 1024).toFixed(1)}MB). Max 5MB.`, 'error');
      fileInput.value = '';
      preview.innerHTML = '';
      return;
    }
  ...
  uploadForm.addEventListener('submit', async (e) => {
    ...
    if (!jobId) return showMessage('upload-status', 'Job ID is required', 'error');
    if (!file) return showMessage('upload-status', 'Please select a photo', 'error');
    ...
    try {
      ...
      showMessage('upload-status', `✓ Photo uploaded!<br><small>Path: ${escapeHTML(data.path)}</small>`, 'success', 0);
      uploadForm.reset();
      preview.innerHTML = '';
      if (typeof refreshDashboard === 'function') setTimeout(refreshDashboard, 1000);
    } catch (err) {
