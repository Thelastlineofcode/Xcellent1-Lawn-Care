...
Replaced all hardcoded Supabase anonymous keys with placeholder text
`YOUR_SUPABASE_ANON_KEY_HERE` in:
...
2. ⏳ Add `<script src="/config.js"></script>` to all HTML files before Supabase
   initialization
...
**Status**: Credentials removed, config pattern documented. HTML files need
`/config.js` script tags added.

**Next Action**: Add `<script src="/config.js"></script>` to all protected HTML
files.
